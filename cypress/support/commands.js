Cypress.Commands.add('calculate', (input1, input2, operator) => { 
 ...
 Cypress.Commands.add('verifyHistory', (input1, input2, operator, row, result, time) => {
  cy.get('[class="table"]')
    .find('tbody')
    .find('tr')
    .eq(row)
    ...
    .then(($timeElem) => {
      ...
      if (time.getSeconds() > 58) {
        ...
        currentTime = `${hours}:${nextMinute < 10 ? '0' + nextMinute : nextMinute}:00 ${ampm}`;
      } else {
      ...
      cy.get('[class="table"]')
        ...
        .invoke('text')
        .then((text) => {
          expect(text.replace(/\n|\s/g, '')).to.eq(`${input1}${operator}${input2}`);
        });
